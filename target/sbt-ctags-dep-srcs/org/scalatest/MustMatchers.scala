 ...
 * <code>must</code> <code>produce</code> syntax. This will both continue to work as before, but will generate a deprecation
 ...
 * the same with one potential exception, which must in practice be extremely rare. The potential breakage is that if you included <code>length</code> or <code>size</code>
 ...
 * from component traits, as <a>described below</a>. Note that you must not see any new implicit conflicts, because we managed to <em>reduce</em> the number
...
*/
  ...
  /**
   * <strong>The <code>evaluating { ... } must produce [...Exception]</code> syntax has been deprecated and
   * will be removed in a future version of ScalaTest. Please use <code>a/an [...Exception] must be
   ...
   * </pre>
   */
  @deprecated("Please use 'an [Exception] must be thrownBy { ... }' syntax instead")
  ...
  def oneOf(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    ...
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("oneOfDuplicate"), getStackDepthFun("MustMatchers.scala", "oneOf"))
  ...
  def atLeastOneOf(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("atLeastOneOfDuplicate"), getStackDepthFun("MustMatchers.scala", "atLeastOneOf"))
  ...
  def noneOf(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("noneOfDuplicate"), getStackDepthFun("MustMatchers.scala", "noneOf"))
  ...
  def only(xs: Any*) = {
    if (xs.isEmpty)
      throw new NotAllowedException(FailureMessages("onlyEmpty"), getStackDepthFun("MustMatchers.scala", "only"))
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("onlyDuplicate"), getStackDepthFun("MustMatchers.scala", "only"))
  ...
  def inOrderOnly[T](firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("inOrderOnlyDuplicate"), getStackDepthFun("MustMatchers.scala", "inOrderOnly"))
  ...
  def allOf(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("allOfDuplicate"), getStackDepthFun("MustMatchers.scala", "allOf"))
  ...
  def inOrder(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("inOrderDuplicate"), getStackDepthFun("MustMatchers.scala", "inOrder"))
  ...
  def atMostOneOf(firstEle: Any, secondEle: Any, remainingEles: Any*) = {
    val xs = firstEle :: secondEle :: remainingEles.toList
    if (xs.distinct.size != xs.size)
      throw new NotAllowedException(FailureMessages("atMostOneOfDuplicate"), getStackDepthFun("MustMatchers.scala", "atMostOneOf"))
...
*/
  ...
  private[scalatest] def doCollected[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, methodName: String, stackDepth: Int)(fun: T => Unit) {
    collected match {
      case AllCollected =>
        doForAll(xs, original, "allShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e => 
          fun(e)
        }
      case AtLeastCollected(num) => 
        doForAtLeast(num, xs, original, "atLeastShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
      case EveryCollected => 
        doForEvery(xs, original, "everyShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
      case ExactlyCollected(num) => 
        doForExactly(num, xs, original, "exactlyShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
      case NoCollected =>
        doForNo(xs, original, "noShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
      case BetweenCollected(from, to) =>
        doForBetween(from, to, xs, original, "betweenShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
      case AtMostCollected(num) =>
        doForAtMost(num, xs, original, "atMostShorthandFailed", "MustMatchers.scala", methodName, stackDepth) { e =>
          fun(e)
        }
    }
  }
   ...
   * @author Chee Seng
   */
  sealed class ResultOfNotWordForCollectedAny[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, mustBeTrue: Boolean) {
    ...
    def contain(only: ResultOfOnlyApplication)(implicit aggregating: Aggregating[T]) {
      ...
      doCollected(collected, xs, original, "contain", 1) { e =>
        if (aggregating.containsOnly(e, right) != mustBeTrue) {
          ...
          throw newTestFailedException(
            FailureMessages(
              (if (mustBeTrue) "didNotContainOnlyElements" else "containedOnlyElements") + postfix,
              e,
              UnquotedString(right.map(FailureMessages.decorateToStringValue).mkString(", "))
            ),
            None,
   ...
   * @author Chee Seng
   */
  final class ResultOfContainWordForCollectedAny[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  sealed class ResultOfBeWordForCollectedAny[T](collected: Collected, xs: scala.collection.GenTraversable[T], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  final class ResultOfBeWordForCollectedArray[T](collected: Collected, xs: scala.collection.GenTraversable[Array[T]], original: Any, mustBeTrue: Boolean)
    extends ResultOfBeWordForCollectedAny(collected, xs, original, mustBeTrue) {
...
org.scalatest.exceptions.TestFailedException: org.scalatest.Matchers$ResultOfCollectedAny@18515783 was not null
	...
	at org.scalatest.Matchers$MustMethodHelper$.mustMatcher(MustMatchers.scala:5529)
	at org.scalatest.Matchers$AnyMustWrapper.must(MustMatchers.scala:5563)
	at .<init>(<console>:15)
	at .<clinit>(<console>)
*/
    ...
    def must(rightMatcher: Matcher[T]) {
      doCollected(collected, xs, original, "must", 1) { e =>
    ...
    def mustEqual(right: Any)(implicit equality: Equality[T]) {
      doCollected(collected, xs, original, "mustEqual", 1) { e =>
    ...
    def mustBe(sortedWord: SortedWord)(implicit sortable: Sortable[T]) {
      doCollected(collected, xs, original, "mustBe", 1) { e =>
     ...
     * </pre>
     */
    def must(beWord: BeWord) = new ResultOfBeWordForCollectedAny[T](collected, xs, original, true)
    ...
    def mustNot[U <: T](rightMatcherX1: Matcher[U]) {
      doCollected(collected, xs, original, "mustNot", 1) { e =>
        val result = 
   ...
   * @author Bill Venners
   */
  final class ResultOfHaveWordForCollectedExtent[A](collected: Collected, xs: scala.collection.GenTraversable[A], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  final class ResultOfStartWithWordForCollectedString(collected: Collected, xs: scala.collection.GenTraversable[String], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  final class ResultOfIncludeWordForCollectedString(collected: Collected, xs: scala.collection.GenTraversable[String], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  final class ResultOfEndWithWordForCollectedString(collected: Collected, xs: scala.collection.GenTraversable[String], original: Any, mustBeTrue: Boolean) {
   ...
   * @author Chee Seng
   */
  final class ResultOfFullyMatchWordForCollectedString(collected: Collected, xs: scala.collection.GenTraversable[String], original: Any, mustBeTrue: Boolean) {
  ...
  // This is where ShouldMustMatchers.scala started 
   ...
   * @author Bill Venners
   */
  sealed class AnyMustWrapper[T](leftSideValue: T) {
    ...
    def must(rightMatcherX1: Matcher[T]) {
      MustMethodHelper.mustMatcher(leftSideValue, rightMatcherX1)
    }
     ...
     */
    def must[TYPECLASS1[_]](rightMatcherFactory1: MatcherFactory1[T, TYPECLASS1])(implicit typeClass1: TYPECLASS1[T]) {
      MustMethodHelper.mustMatcher(leftSideValue, rightMatcherFactory1.matcher)
    }
     ...
     */
    def must[TYPECLASS1[_], TYPECLASS2[_]](rightMatcherFactory2: MatcherFactory2[T, TYPECLASS1, TYPECLASS2])(implicit typeClass1: TYPECLASS1[T], typeClass2: TYPECLASS2[T]) {
      MustMethodHelper.mustMatcher(leftSideValue, rightMatcherFactory2.matcher)
    }
     ...
     * </pre>
     */
    def must(notWord: NotWord): ResultOfNotWordForAny[T] = new ResultOfNotWordForAny[T](leftSideValue, false)
     ...
     * </pre>
     */
    def must(beWord: BeWord): ResultOfBeWordForAny[T] = new ResultOfBeWordForAny(leftSideValue, true)
     ...
     * </pre>
     */
    def mustNot(beWord: BeWord): ResultOfBeWordForAny[T] = new ResultOfBeWordForAny(leftSideValue, false)
    ...
    def mustNot(rightMatcherX1: Matcher[T]) {
      MustMethodHelper.mustNotMatcher(leftSideValue, rightMatcherX1)
    }
     ...
     */
    def mustNot[TYPECLASS1[_]](rightMatcherFactory1: MatcherFactory1[T, TYPECLASS1])(implicit typeClass1: TYPECLASS1[T]) {
      MustMethodHelper.mustNotMatcher(leftSideValue, rightMatcherFactory1.matcher)
    }
...
*/
   ...
   * @author Bill Venners
   */
  final class StringMustWrapper(val leftSideString: String) extends AnyMustWrapper(leftSideString) with StringMustWrapperForVerb {
     ...
     * </pre>
     */
    def mustNot(compileWord: CompileWord): Unit = macro CompileMacro.mustNotCompileImpl
